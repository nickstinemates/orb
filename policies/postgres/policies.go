...
func (r policiesRepository) RetrieveTotalPoliciesByOwner(ctx context.Context, owner string) (int, error) {
	q := fmt.Sprintf(`SELECT count(*) FROM policies WHERE mf_owner_id = :mf_owner_id`)
